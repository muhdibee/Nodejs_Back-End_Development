  ...
  req.on("end", () => {
    ...
    fs.readFile(bookDbPath, "utf-8", (err, data) => {
      if (err) {
        ...
        const IndexOfbookToUpdate = books.findIndex(
          (book) => book.id == bookDetailsToUpdate.id
        );
  ...
  req.on("end", () => {
    ...
    fs.readFile(bookDbPath, "utf-8", (err, data) => {
      if (err) {
        ...
        const books = JSON.parse(data);
        const IndexOfbookToDelete = books.findIndex(
          (book) => book.id == bookDetailsToDelete.id
        );
